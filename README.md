...
7. **大致成本**：内置 37 个模型 ¥/M token 的价目表（输入 / 输出 / 缓存输入），公式 = `(未缓存输入 + 缓存写) × 输入价 + (输出 + reasoning) × 输出价 + 缓存读 × 缓存价`（原始 `input_tokens` 已含缓存读，输入桶须先减去）。模型名匹配链：标记 → 精确 → 归一化（去 provider 前缀、小写）→ 内置别名（`stealth/ox-alpha` → `GLM-5.3-Flash`）→ **默认按 deepseek-v4-pro**。表里没列的模型都会按 deepseek-v4-pro 算，**这是估值，不是真实账单**。
...
│   ├── trend.tsx         # 趋势（Token / 成本 模式切换 + 粒度切换）
